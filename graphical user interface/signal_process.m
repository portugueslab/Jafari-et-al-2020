function [signal_processed,signal_events,signal_bump] = signal_process(data,parameters,do_preprocess,do_events)
% Process the signal extracted from the ROI.
%
%              DATA (structure extracted from signal_info)
% ==========================================================================
% input variable DATA has the following fields:
%
%           type = Neuron, Glial cell ,Blood Vessel, Neuropile
%     frame_rate = in Herz
%     ROI_pixels = # pixels in ROI for event estimation
%         signal = raw signal inside the ROI
%      neuropile = raw signal surrounding the ROI
%      processed = results of this code
%         events = results of this code
%
%            PARAMETERS (structure contained in gui_info.signal)
% ==========================================================================
%
% --------- Neuropile subtraction ---------
% NP_fraction = fraction of neuropile signal to be subtracted
%
% --------- Baseline (Fo) calculation ---------
% Fo_range = range over which the baseline of the signal is define for each frame
% Fo_quantile = quantile in the signal range determining the baseline
%
% --------- Filtering of dF/F ---------
% Filter_exp = filter of the dF/F (on top a 3 frame average that is not changeable)
%
% --------- Threshold to accept event ---------
% probability_threshold = max bump probability that comes from Vogelstein routine
% signal_threshold = minimal signal that has to be passed by the Ca-bump
%               -> a 2*std(signal(no_event)) is used if value above threshold
%
%                            OPTIONS (logical)
% ==========================================================================
% Parts of the processing can be jumped if needed
% do_preprocess = update processed signal from input
% do_events = update event estimation from input
%
%% ==========================================================================
...
if do_preprocess
    ...
    % dF/F signal
    min_size_half = round((parameters.Fo_range*data.frame_rate)/2);
    Fo = zeros(size(data.signal(1,:,1)));
    for frame=1:length(data.signal(1,:,1))
        tmp = sort(data.signal(:,max(1,frame-min_size_half) : min(length(data.signal(1,:,1)),frame+min_size_half),1));
        Fo(:,frame) = tmp(1+floor(parameters.Fo_quantile*length(tmp)));
    end
    signal= (data.signal(1,:,1)-Fo)./Fo;
    
    % dF/F neuropile
    min_size_half = round((parameters.Fo_range*data.frame_rate)/2);
    Fo = zeros(size(data.neuropile));
    for frame=1:length(data.neuropile)
        tmp = sort(data.neuropile(:,max(1,frame-min_size_half) : min(length(data.neuropile),frame+min_size_half),1));
        Fo(:,frame) = tmp(1+floor(parameters.Fo_quantile*length(tmp)));
    end
    neuropile = (data.neuropile-Fo)./Fo;
        
    % ---------------- Filter dF/F
    
    if parameters.Filter>0
        
        % do a small filter first
        w_filter = ones(1,3)/3;
        signal= conv(signal,w_filter,'same');
        neuropile = conv(neuropile,w_filter,'same');
        
%         % --- average filter
%         w_filter = ones(1,ceil(data.frame_rate*parameters.Filter));
%         
%         % --- Gaussian filter
%         w_filter = exp(-(0:length(data.signal)-1).^2/(2*(data.frame_rate*parameters.Filter).^2));
%         w_filter(w_filter<0.005)=[];
%         w_filter = [w_filter(end:-1:2),w_filter];

        % --- exponential filter
        w_filter = exp(-(0:length(signal)-1)/(data.frame_rate*parameters.Filter));
        w_filter(w_filter<0.005)=[];
        w_filter = [zeros(1,length(w_filter)),w_filter];
    ...
    % ---------------- Neuropile correction and get to percent
    
    data.processed = (signal - parameters.NP_fraction * neuropile);
    
    % ---------------- RETURN
    signal_processed = data.processed;
    
end

%% CALCULATE EVENTS
% use old data.processed and get events

if do_events
    
    % ---------------- Event propability using fast_oopsi
    
    % event probability using fast_oopsi
    V = struct;
    V.dt = 1/data.frame_rate;
    V.Ncells = 1;
    V.Npixels = data.ROI_pixels;
    spike_prob = fast_oopsi(data.processed,V);
    
    % --------------- Detect events
    
    % separate the bumps
    spike_prob(spike_prob<quantile(spike_prob,.85))=0;  %--> check quantile!
    ind_zero = find(spike_prob==0);
    ind_bump_begin = ind_zero([diff(ind_zero);1]>1);
    ind_bump_end = ind_zero([1;diff(ind_zero)]>1);
    ind_bump_end(ind_bump_end>length(data.processed))=length(data.processed);
    
    % if NAN, get signal threshold from unfiltered signal where prob = 0
    if isnan(parameters.signal_threshold)
        parameters.signal_threshold =  max([0.1 3*std(data.processed(ind_zero))]);
    end
    
    % get bump max probability and test if signal threshold is crossed
    is_in_th = false(size(ind_bump_begin));
    ind_bump_prob = zeros(size(ind_bump_begin));
    for bump = 1:length(ind_bump_begin)
        is_in_th(bump) = ~isempty(find(data.processed(ind_bump_begin(bump):ind_bump_end(bump))>parameters.signal_threshold));
        [ind_bump_prob(bump),location_shift]=max(spike_prob(ind_bump_begin(bump):ind_bump_end(bump)));
        % shift bum begin to maximum of probability
        ind_bump_begin(bump) = ind_bump_begin(bump)+location_shift-1;
    end
    
    % if NAN, get probability threshold quantile
    if isnan(parameters.probability_threshold)
        parameters.probability_threshold = quantile(spike_prob(spike_prob>0),.95);
    end
        
    % Threshold bump probability
    test = ind_bump_prob>=parameters.probability_threshold & is_in_th;
    
    % -----------------------  RETURN
    signal_events = [ind_bump_begin(test),ind_bump_prob(test)];
    signal_bump = [];
    signal_bump.frame = ind_bump_begin;
    signal_bump.prob = ind_bump_prob;
end
...
%% ====== Joshua Vogelstein fast_oopsi routine ==============
